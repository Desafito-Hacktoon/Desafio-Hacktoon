...
export class ZardPopoverDirective implements OnInit, OnDestroy {
  ...
  show() {
    ...
    // Aplicar z-index e background branco após anexar
    if (this.overlayRef?.overlayElement) {
      this.overlayRef.overlayElement.style.zIndex = '10000';
      const popoverElement = this.overlayRef.overlayElement.querySelector('z-popover') as HTMLElement;
      if (popoverElement) {
...
export class ZardPopoverComponent {
  ...
  protected readonly classes = computed(() => mergeClasses(popoverVariants(), this.class(), '!bg-white !z-[10000]'));
}
