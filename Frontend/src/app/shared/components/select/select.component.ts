...
@Component({
  ...
  template: `
    ...
    <ng-template #dropdownTemplate>
      <div [class]="contentClasses()" role="listbox" [attr.data-state]="'open'" (keydown)="onDropdownKeydown($event)" tabindex="-1" class="!bg-white !z-[10000] shadow-lg">
...
export class ZardSelectComponent implements ControlValueAccessor, AfterContentInit, OnDestroy {
  ...
  private open() {
    ...
    // Aplicar z-index e estilos diretamente no overlay
    if (this.overlayRef.overlayElement) {
      this.overlayRef.overlayElement.style.zIndex = '10000';
      const listbox = this.overlayRef.overlayElement.querySelector('[role="listbox"]') as HTMLElement;
      if (listbox) {
        listbox.style.backgroundColor = 'white';
        listbox.style.zIndex = '10000';
      }
    }
    ...
    if (isPlatformBrowser(this.platformId)) {
      try {
        ...
        this.overlayRef = this.overlay.create({
          ...
          panelClass: 'zard-select-overlay',
        });
        
        // Aplicar z-index alto ao overlay
        if (this.overlayRef.overlayElement) {
          this.overlayRef.overlayElement.style.zIndex = '10000';
        }
      } catch (error) {
